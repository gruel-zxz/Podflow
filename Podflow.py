...
# 获取已下载视频时长模块
def get_duration_ffprobe(file_path, write_in_log=True):
    try:
        # 调用 ffprobe 获取视频文件的时长信息
        ...
        if write_in_log:
            ...
            print(f"{datetime.now().strftime('%H:%M:%S')}|\033[31mError:\033[0m {error_note}")
        return None
...
def bulid_Netscape_HTTP_Cookie(file_name, cookie={}):
    b_3 = http_client("https://www.bilibili.com", "哔哩哔哩主页", 10, 4, True).cookies.get_dict()
...
.bilibili.com	TRUE	/	FALSE	0	buvid3	{b_3.get("buvid3", "")}
.bilibili.com	TRUE	/	FALSE	0	b_nut	{b_3.get("b_nut", "")}'''
