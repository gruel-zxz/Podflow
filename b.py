...
def create_custom_http_server(port=8000, allowed_paths=["YouTube.xml", "channel_audiovisual", "channel_rss"], valid_token="password"):
    class CustomHTTPRequestHandler(http.server.SimpleHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            auth_token = query_components.get('token', [None])[0]
            if not auth_token or auth_token != valid_token:
                self.send_error(403, "Forbidden: Invalid or missing token")  # 返回403禁止访问
                return

            translated_path = self.translate_path(urlparse(self.path).path)
        ...
        def log_message(self, format, *args):
            log = "%s %s" % (self.requestline, str(args[1]))
            print(log)
...
# 创建并运行自定义HTTP服务器，允许访问的路径为 "YouTube.xml", "channel_audiovisual", "channel_rss", "Podflow.xml"，并添加token认证
create_custom_http_server(allowed_paths=["YouTube.xml", "channel_audiovisual", "channel_rss", "Podflow.xml"], valid_token="a123456")
